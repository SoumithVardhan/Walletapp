...
const BannerHeadline = () => {
  return (
    <div className="text-center w-full lg:w-1/2 flex sm:flex-col flex-col justify-start gap-5">
      ...
      <h1 className="w-full text-5xl text-black gap-5 text-center">Simplify Your</h1>
      <h1 className="text-5xl text-[#6a51a6]">
        <Typewriter strings={["Payments"]} typeSpeed={200} loop />
      </h1>
    </div>
  );
};
...
const Typewriter: React.FC<TypewriterProps> = ({ strings, typeSpeed, loop }) => {
  const [text, setText] = useState("");
  const [index, setIndex] = useState(0);
  const [charIndex, setCharIndex] = useState(0);

  useEffect(() => {
    const currentString = strings?.[index]; // Safely access the current string
    ...
    } else if (loop && strings?.length) {
      const loopTimeout = setTimeout(() => {
        setText("");
        setCharIndex(0);
        setIndex((prev) => (prev + 1) % strings.length);
      }, typeSpeed * 10); // Delay before restarting typing

      return () => clearTimeout(loopTimeout);
    }
  }, [charIndex, index, loop, strings, typeSpeed]);
  ...
  return (
    <div className="w-full">
      <div className="relative mx-auto border-gray-800 dark:border-gray-800 bg-gray-800 border-[8px] rounded-t-xl h-[294px] max-w-[472px]">
        <div className="relative rounded-lg overflow-hidden h-[278px] bg-white bg-[url('./ss.png')] bg-no-repeat bg-cover bg-blend-multiply"></div>
      </div>
...
const MobileUi = () => {
  return (
    <div className="relative mx-auto border-gray-800 dark:border-gray-800 bg-gray-800 border-[14px] rounded-[2.5rem] h-[400px] w-[230px]">
      ...
      <div className="rounded-[2rem] overflow-hidden h-[372px] bg-white bg-[url('./demo.gif')] bg-no-repeat bg-cover bg-blend-multiply"></div>
    </div>
  );
};
